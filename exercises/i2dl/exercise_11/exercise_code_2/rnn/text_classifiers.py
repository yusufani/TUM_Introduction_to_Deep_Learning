import torch.nn as nn
from torch.nn.utils.rnn import pack_padded_sequence
import torch
from .rnn_nn import Embedding, RNN, LSTM


class RNNClassifier(nn.Module):
    def __init__(self, num_embeddings, embedding_dim, hidden_size, use_lstm=True, **additional_kwargs):
        """
        Inputs:
            num_embeddings: size of the vocabulary
            embedding_dim: size of an embedding vector
            hidden_size: hidden_size of the rnn layer
            use_lstm: use LSTM if True, vanilla RNN if false, default=True
        """
        super().__init__()

        # Change this if you edit arguments
        hparams = {
            'num_embeddings': num_embeddings,
            'embedding_dim': embedding_dim,
            'hidden_size': hidden_size,
            'use_lstm': use_lstm,
            **additional_kwargs
        }
        if 'device' not in hparams:
            hparams['device'] = 'cpu'
        # if you do not inherit from lightning module use the following line
        self.hparams = hparams

        # if you inherit from lightning module, comment out the previous line and use the following line
        # self.hparams.update(hparams)

        ########################################################################
        # TODO: Initialize an RNN network for sentiment classification         #
        # hint: A basic architecture can have an embedding, an rnn             #
        # and an output layer                                                  #
        ########################################################################
        self.hparams['num_layers'] = 3
        self.embedding = nn.Embedding(self.hparams['num_embeddings'], self.hparams['embedding_dim'], padding_idx=0)
        self.lstm = nn.LSTM(self.hparams['embedding_dim'], self.hparams['hidden_size'], num_layers=self.hparams['num_layers'], batch_first=False)
        self.out = nn.Sequential(
            nn.Linear(self.hparams['hidden_size'], self.hparams['hidden_size'] // 2),
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(self.hparams['hidden_size'] // 2, 1)
        )
        self.hidden = None
        ########################################################################
        #                           END OF YOUR CODE                           #
        ########################################################################

    def init_hidden(self, batch_size):
        state_size = (self.hparams['num_layers'], batch_size, self.hparams['hidden_size'])
        return torch.zeros(state_size, device=self.hparams['device']), torch.zeros(state_size, device=self.hparams['device'])

    def forward(self, sequence, lengths=None):
        """
        Inputs
            sequence: A long tensor of size (seq_len, batch_size)
            lengths: A long tensor of size batch_size, represents the actual
                sequence length of each element in the batch. If None, sequence
                lengths are identical.
        Outputs:
            output: A 1-D tensor of size (batch_size,) represents the probabilities of being
                positive, i.e. in range (0, 1)
        """
        output = None

        ########################################################################
        # TODO: Apply the forward pass of your network                         #
        # hint: Don't forget to use pack_padded_sequence if lenghts is not None#
        # pack_padded_sequence should be applied to the embedding outputs      #
        ########################################################################
        #if self.hidden is None or self.hidden[0].size(1) != sequence.size(1):
        #    self.hidden = self.init_hidden(sequence.size(1))
        embedded_sequence = self.embedding(sequence)
        if lengths != None:
            embedded_sequence = pack_padded_sequence(embedded_sequence, lengths)

        if self.hparams['use_lstm']:
            state_size = (self.hparams['num_layers'], sequence.size(1), self.hparams['hidden_size'])
            # print(f'shape of hidden: {state_size}')
            b, (h, a) = self.lstm(embedded_sequence,
                                  (torch.zeros(state_size, device=self.hparams['device']), torch.zeros(state_size, device=self.hparams['device'])))
        h_last = h[-1]  # get last LSTM layer output

        output = self.out(h_last)
        output = nn.Sigmoid()(output).squeeze(1).view(-1)

        #self.hidden = (h.detach(), a.detach())  # Not working :/
        #self.hidden = self.init_hidden(sequence.size(1))
        ########################################################################
        #                           END OF YOUR CODE                           #
        ########################################################################

        return output
