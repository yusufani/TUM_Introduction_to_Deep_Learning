...
class LeakyRelu:
    ...
    def forward(self, x):
        ...
        outputs = np.maximum(x, 0) + self.slope * np.minimum(x, 0)
        cache = x
    ...
    def backward(self, dout, cache):
        ...
        ########################################################################
        dx = dout * (cache > 0) + dout * self.slope * (cache <= 0)
        # if x was higher than 0, then the derivative is 1, otherwise it is the slope
