...
class MyOwnNetwork(ClassificationNet):
    ...
    def __init__(self, activation=Sigmoid(), num_layer=2,
        ...
        super(ClassificationNet, self).__init__("cifar10_classification_net")
        heuristic_initialization = kwargs.pop('heuristic_initialization', False)
        self.activation = activation
        self.reg_strength = reg
        ...
        if heuristic_initialization:
            ...
            self.params = {'W1': np.random.randn(input_size, hidden_size) * np.sqrt(2 / (hidden_size * hidden_size)),
                           'b1': np.zeros(hidden_size)}
        ...
        for i in range(num_layer - 2):
            if heuristic_initialization:
                ...
                self.params['W' + str(i + 2)] = np.random.randn(hidden_size,
                                                                hidden_size) * np.sqrt(2 / (hidden_size * hidden_size))
            else:
                self.params['W' + str(i + 2)] = std * np.random.randn(hidden_size,
                                                                      hidden_size)
            self.params['b' + str(i + 2)] = np.zeros(hidden_size)
        ...
        if heuristic_initialization:
            self.params['W' + str(num_layer)] = np.random.randn(hidden_size,
                                                                num_classes) * np.sqrt(2 / (num_classes * num_classes))
        else:
            self.params['W' + str(num_layer)] = std * np.random.randn(hidden_size,
                                                                      num_classes)
