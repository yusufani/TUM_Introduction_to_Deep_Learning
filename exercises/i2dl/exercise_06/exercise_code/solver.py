    ...
    def train(self, epochs=100, patience=None):
        ...
        for t in range(epochs):
            ...
            for batch in self.train_dataloader:
                ...
                validate = t == 0
                train_loss, train_correct = self._step(X, y, validation=validate)
                train_corrects.extend(train_correct)
            ...
            for batch in self.val_dataloader:
                ...
                val_loss, val_correct = self._step(X, y, validation=True)
                self.val_batch_loss.append(val_loss)
                val_corrects.extend(val_correct)
            ...
            if self.verbose and t % self.print_every == 0:
                print('(Epoch %d / %d) train loss: %f; train acc: %f val loss: %f val acc: %f' % (
                    t + 1, epochs, train_epoch_loss, np.array(train_corrects).mean(), val_epoch_loss,
                    np.array(val_corrects).mean()))
