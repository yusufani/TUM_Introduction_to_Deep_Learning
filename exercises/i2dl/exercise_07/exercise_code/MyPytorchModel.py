...
class MyPytorchModel(pl.LightningModule):
    ...
    def __init__(self, hparams):
        ...
        self.model = nn.Sequential(
            nn.Linear(hparams['input_size'], self.hparams["n_hidden"]),
            nn.LeakyReLU(),
            nn.Linear(self.hparams["n_hidden"], self.hparams["n_hidden"] // 2),
            nn.LeakyReLU(),
            nn.Linear(self.hparams["n_hidden"] // 2, self.hparams["n_hidden"] // 4),
            nn.LeakyReLU(),
            nn.Linear(self.hparams["n_hidden"] // 4, self.hparams["n_hidden"] // 2),
            nn.Dropout(0.5),
            nn.LeakyReLU(),
            nn.Linear(self.hparams["n_hidden"] // 2, hparams["n_classes"]),
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log('loss', loss)
        self.log('Train acc', n_correct / n_total, logger=True, prog_bar=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log('Val acc', n_correct / n_total, logger=True, prog_bar=True)
    ...
    def configure_optimizers(self):
        ...
        lr_scheduler = torch.optim.lr_scheduler.LambdaLR(optim , lr_lambda=lambda epoch: 0.95 ** epoch)
