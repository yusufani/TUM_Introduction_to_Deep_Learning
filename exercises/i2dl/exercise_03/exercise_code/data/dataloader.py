...
class DataLoader:
    ...
    def __iter__(self):
        ...
        def prepare_batch(batch):
            return batch_to_numpy(combine_batch_dicts(batch))
        if self.shuffle:
        ...
        b_count = 0
        batch = []
        for count, index in enumerate(index_iterator):  # iterate over indices using the iterator
            batch.append(self.dataset[index])

            if len(batch) == self.batch_size:
                yield prepare_batch(batch)  # use yield keyword to define a iterable generator
                batch = []
                b_count += 1
    ...
    def __len__(self):
        ...
        if self.drop_last:
            ...
            length = (len(self.dataset)//self.batch_size) + 1 if  len(self.dataset)%self.batch_size != 0 else 0
